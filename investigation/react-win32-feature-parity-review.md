...
| Graph selection | Stable node/edge selection synchronized with sidebar/workspace | Client state only | **Missing** | Single selection store keyed by project path + node/edge ID | P0 | M | Click, keyboard and sidebar select the same identity across refreshes |
...
| Node inspector | Win32 information/actions are fragmented across card, context menu, forms, loop bar and workspace | Most fields already in `LoopNode`; mailbox is separate | **Missing** | Persistent right-side inspector described below | P0 | L | Selecting a node exposes complete supported data/actions without opening a modal |
| Node state and attention | Idle/running/awaiting/blocked/waiting/succeeded/failed/stalled/stopped, presence, active dependents, reasons | `LoopNode.state`, `presence`, `stallReason`, `launchFailure`, `resolution`, edges | **Partial**: state label only | Inspector status header plus accessible reason text | P0 | M | Every enum case has distinct label, color-independent icon and explanation |
...
| Node stop | Context/menu/shortcut | Existing `stopNode` | **Missing** | Visible action when unresolved; confirm only where destructive side effects warrant | P0 | S after command bus | Correlated refusal shown; node remains and transcript survives |
| Node restart/resume | Daemon supports restart/resume; Win32 context menu does not expose restart | Existing `restartNode`, `resumeSession`, `restartSessions` | **Missing** | Inspector overflow action with lifecycle explanation | P1 | M | Restart waits for authoritative `sessionRestarts`/snapshot before reattach |
| Node rename | Context menu and F2 | Existing `renameNode` | **Missing** | Inline inspector title edit and context command | P1 | S | Blank title refused; stable node/session identity retained |
| Node edit | Win32 modal edits goal/predicate/poll/stall/metric/trigger/check/model subset | Existing `updateNode(NodeUpdate)` | **Missing** | Section-level Edit in inspector; submit partial update | P0 | M | Only changed fields sent; empty update refused; self-stop-condition rules surfaced |
| Node delete | Context action with cancellation-default warning | Existing `deleteNode` | **Missing** | Inspector danger zone and contextual menu | P1 | S | Explicit project/node identity in confirmation; selection moves predictably |
...
| Usage and metrics | Usage refresh, usage samples, metric history/sparklines | Existing node usage/metric fields and `refreshUsage` | **Missing** | Inspector Metrics section with explicit Refresh | P1 | M | Refresh is user-triggered; no estimated usage is presented as reported |
...
| Accessibility structure | Custom UIA provider exposes roles/patterns/dynamic commands | No daemon work | **Partial**: semantic shell and SVG names only | Native HTML semantics, roving focus, textual graph outline, live regions | P0 continuously | L; WebView/Narrator variance | Axe + keyboard + Narrator gates; all actions reachable without pointer |
| Keyboard shortcuts | Native accelerators and centralized `InputRouter` | No daemon work | **Missing** | One command registry drives native menu, React UI and palette | P0 | M | Conflicts suppressed in text fields; shortcuts discoverable and testable |
