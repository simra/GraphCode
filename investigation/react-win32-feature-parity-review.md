...
| Edge creation/edit/delete | Drag connector and modal spec editor; context actions | Existing `createEdge`, `deleteEdge`; no atomic `updateEdge` command | **Partial**: pointer connector drag prefills the registry-driven New Edge dialog, which maps endpoint, kind, condition, payload transform, bounded cycle guard and spawn target to the exact existing spec; keyboard selection/dialog alternatives and stable-ID deletion remain available. Existing-edge edits remain blocked by DT-002 | Add atomic edit only after DT-002 | P1 | L | Keyboard alternative exists; invalid cycles/specs show daemon refusal; edit preserves edge identity |
