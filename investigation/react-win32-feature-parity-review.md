...
| Recent projects | Native File submenu and sidebar recent/open distinction | Existing `listRecentProjects`, `recentProjectsListed` | **Implemented**: Overview, alphabetized Open projects and recency-sorted unopened Recent projects are distinct; selecting a recent project opens it authoritatively and then selects its canonical path | Add project row overflow lifecycle actions | P1 | S | Recent unopened projects remain selectable; open projects retain graph state |
