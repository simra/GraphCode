...
| Sidebar hierarchy | Local/remote groups, project disclosure, edge-derived nested loop tree, attention/activity | Graph snapshots; some layout state is client-local | **Partial**: the selected project expands into a semantic nested root/composite loop hierarchy with stable-ID selection, explicit composite entry, current-path expansion, and canvas/breadcrumb/inspector synchronization; local/remote grouping and aggregated attention remain | Add local/remote grouping and attention/activity aggregation when authoritative sources are available | P1 | L; cycles and focus restoration | Snapshot reorder does not lose selection/expansion/focus |
...
| Accessibility structure | Custom UIA provider exposes roles/patterns/dynamic commands | No daemon work | **Partial**: semantic shell/inspector, named SVG graph, spatial roving node focus, discoverable keyboard instructions, keyboard edge selection, viewport controls, textual graph outline, color-independent node/edge state patterns, and an axe-core/jsdom regression suite are implemented; concrete Windows Narrator/touch/DPI/contrast gates are documented but still require manual packaged-app execution | Add live regions and execute the documented Narrator matrix in release validation | P0 continuously | L; WebView/Narrator variance | Axe + keyboard + Narrator gates; all actions reachable without pointer |
