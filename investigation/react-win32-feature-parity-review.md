...
| DT-009 | Nested Mailroom ownership and routing | Project-wide versus subgraph-local mailbox authority is unresolved |
| DT-010 | Local/SSH/Codespace project classification | `ProjectRef` exposes no authoritative location kind |
...
The React client is now a usable daemon-backed graph frontend rather than a read-only
demonstration. It has persistent v2 connectivity, authoritative project and Quick Chat
navigation, typed command execution, complete supported node mutations, composite
drill-in, top-level Mailroom, pointer/keyboard graph editing, persisted viewport and
node layout, selected-edge inspection/deletion, and the accessibility behavior needed
for keyboard daily use.

The practical frontend-only and existing-protocol parity work prioritized by this
review is complete. Remaining replacement blockers belong to daemon authority, terminal
streaming, native ingress/filesystem/settings/lifecycle integration, packaging/manual
release validation, or separately scoped product enhancements such as cross-project
lane/search views, a unified Review Needs You queue, context menus, a shortcut
reference, onboarding, and richer metrics visualization.
...
| Graph layout persistence | Per-project canvas pan/zoom and node layout | Client-local `CanvasLayoutStore` | **Implemented**: a validated version-1 Tauri app-data store atomically persists root/composite viewports and pointer/Alt+Arrow node positions by project; Reset Layout restores deterministic placement, and corrupt, oversized, or unsupported data fails visibly | Keep schema migration and corruption behavior in release gates | P1 | M | Restart restores layout; invalid entries fail visibly without overwriting them |
| Node inspector | Win32 information/actions are fragmented across card, context menu, forms, loop bar and workspace | Most fields already in `LoopNode`; mailbox is separate | **Partial**: responsive inspector shows all snapshot-backed overview, brief, goal/metrics/budget, usage, worktree, attachments, activity/summary/board, Mailroom, composite and provenance fields; all established lifecycle/edit/message/memo/refine/rollback/pilot/arm actions are wired. DT-001 memory/playbook reads remain blocked | Add readers only after DT-001 and richer visualizations as separate enhancements | P0 | M | Selecting a node exposes complete supported data/actions without opening a modal |
...
| Node state and attention | Idle/running/awaiting/blocked/waiting/succeeded/failed/stalled/stopped, presence, active dependents, reasons | `LoopNode.state`, `presence`, `stallReason`, `launchFailure`, `resolution`, edges | **Partial**: inspector details and color-independent canvas state are implemented; project rows aggregate presence-corrected failed, stalled, awaiting-input and stranded-blocked loops using the established domain rules. A unified cross-project Review Needs You queue remains separately scoped | Add cross-project review navigation only as a deliberate command/destination enhancement | P0 | M | Every enum case has distinct label, color-independent icon and explanation |
...
| Memo and playbook | Memo command exists; refine/rollback supported by daemon; no unified native viewer | Existing `memoNode`, `refineNode`, `rollbackRefinement`; no memory-read event | **Partial**: memo, complete-playbook refine, and rollback actions use the existing nested command path and correlated daemon confirmation; the UI explicitly states that DT-001 blocks current playbook and history reads | Add readers only after DT-001; do not fabricate current/history content | P1 | L | Memo/refine commands work; history is not fabricated when unavailable |
...
| Composite loops | Create composite, open group, pilot once, arm schedule, nested command routing | Existing subgraph, `subGraphCommand`, `pilotComposite`, `armComposite` | **Implemented** for established wire shapes: New Loop creates composites; sidebar, canvas and breadcrumbs drill into nested graphs; supported mutations route through the full parent chain; Pilot and Arm use exact commands and authoritative gating | Keep nested Mailroom unavailable until DT-009 resolves | P1 | L | Arm disabled until piloted; nested mutations address parent chain correctly |
| Edge creation/edit/delete | Drag connector and modal spec editor; context actions | Existing `createEdge`, `deleteEdge`; no atomic `updateEdge` command | **Partial**: pointer connector drag prefills the registry-driven New Edge dialog, which maps the complete existing spec; keyboard selection/dialog alternatives remain, and selected edges open a dedicated snapshot-backed inspector for endpoints, kind, condition, transforms, cycle/spawn metadata, fire count, stable identity, and authoritative stable-ID deletion. Existing-edge edits remain blocked by DT-002 | Add atomic edit only after DT-002 | P1 | L | Keyboard alternative exists; invalid cycles/specs show daemon refusal; edit preserves edge identity |
...
| Mailroom | Digest in graph, mailbox query, post/watch commands | Existing `mailbox`, `mailroomPost`, `mailroomWatch`; mailbox response event | **Partial**: project Mailroom destination, bounded board/search reads, single-post deep reads, digest/cursor metadata, accessible posting, and node unread/watch controls are implemented against top-level project authority | Resolve DT-009 before exposing Mailroom inside composite graphs | P1 | M | Digest/unread count matches mailbox; watch state and errors are authoritative |
| Activity/summary/board | Native sidebar/activity surfaces; node carries activity, summary and board | Existing `LoopNode` fields and deltas | **Implemented** for current snapshot shapes: the inspector renders activity text, structured summary/pass data and board fields without arbitrary HTML | Treat richer diagrams/history as a separately scoped visualization enhancement | P1 | L; Mermaid/security | Updates do not rerender terminal/shell; inaccessible diagrams have text alternative |
...
| Accessibility structure | Custom UIA provider exposes roles/patterns/dynamic commands | No daemon work | **Partial**: semantic shell/inspectors, named SVG graph, spatial roving node focus, keyboard edge selection, viewport controls, textual graph outline, color-independent state, deduplicated authoritative live announcements, and shared dialog initial focus/trapping/Escape/restoration are implemented with axe-core/jsdom regression gates. No packaged WebView driver, bundled test artifact or interactive Windows CI harness exists, so Narrator/touch/DPI/contrast remain documented manual release gates | Establish packaging infrastructure before claiming real-WebView automation; execute the documented Narrator matrix meanwhile | P0 continuously | L; WebView/Narrator variance | Axe + keyboard + Narrator gates; all actions reachable without pointer |
...
| Persistence | Open projects daemon-owned; UI layout/expansion/workspace settings client-owned | Mixed daemon and client stores | **Partial**: daemon-owned project restore is preserved; a bounded, validated, atomic version-1 Tauri app-data store persists per-project/root-or-composite viewport and node positions. Expansion, workspace and broader preferences remain separately scoped | Extend the versioned client store only for clearly client-owned state | P1 | M | Corrupt local state cannot corrupt daemon graph and is recoverable |
