...
| Edge creation/edit/delete | Drag connector and modal spec editor; context actions | Existing `createEdge`, `deleteEdge`; no atomic `updateEdge` command | **Partial**: registry-driven New Edge dialog maps endpoint, kind, condition, payload transform, bounded cycle guard and spawn target to the exact existing spec; canvas edges are keyboard-selectable and stable-ID edges can be deleted after confirmation. Existing-edge edits remain blocked by DT-002 | Add pointer connector drag as a shortcut and atomic edit only after DT-002 | P1 | L | Keyboard alternative exists; invalid cycles/specs show daemon refusal; edit preserves edge identity |
...
| Mailroom | Digest in graph, mailbox query, post/watch commands | Existing `mailbox`, `mailroomPost`, `mailroomWatch`; mailbox response event | **Partial**: typed bounded board reads, full-body refresh, digest count, accessible post composer with authoritative UTF-8 limits, and correlated errors are implemented in the inspector; unread cursor/search/deep-read/watch remain | Add project Mailroom destination plus node-specific unread/watch controls | P1 | M | Digest/unread count matches mailbox; watch state and errors are authoritative |
