...
| Sidebar hierarchy | Local/remote groups, project disclosure, edge-derived nested loop tree, attention/activity | Graph snapshots; some layout state is client-local | **Partial**: the selected project expands into a semantic nested root/composite loop hierarchy with stable-ID selection, explicit composite entry, current-path expansion, and canvas/breadcrumb/inspector synchronization. Current `ProjectRef` has no location kind, so local/remote grouping is blocked on DT-010; attention aggregation remains separately scoped | Add grouping only after DT-010 and consider attention aggregation as a separate UX enhancement | P1 | L; cycles and focus restoration | Snapshot reorder does not lose selection/expansion/focus |
...
| Accessibility structure | Custom UIA provider exposes roles/patterns/dynamic commands | No daemon work | **Partial**: semantic shell/inspectors, named SVG graph, spatial roving node focus, discoverable keyboard instructions, keyboard edge selection, viewport controls, textual graph outline, color-independent state patterns, a deduplicated atomic live region for authoritative connection/mutation/selection changes, and an axe-core/jsdom suite are implemented; concrete Windows Narrator/touch/DPI/contrast gates still require manual packaged-app execution | Execute the documented Narrator matrix in release validation | P0 continuously | L; WebView/Narrator variance | Axe + keyboard + Narrator gates; all actions reachable without pointer |
