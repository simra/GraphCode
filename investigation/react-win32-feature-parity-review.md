...
| Graph selection | Stable node/edge selection synchronized with sidebar/workspace | Client state only | **Partial**: node selection is keyed by project/node ID, survives snapshots, clears on removal, and supports click, spatial arrows, Home/End, Enter and Space; edges are keyboard selectable, while sidebar/workspace synchronization remains | Extend the same selection model to sidebar and workspace | P0 | M | Click, keyboard and sidebar select the same identity across refreshes |
| Graph pan/zoom/fit | Pointer pan, wheel/pinch anchored zoom, zoom controls, persisted canvas layout | Client-local persistence | **Partial**: pointer background pan, wheel zoom, visible controls, registry/native-menu shortcuts for zoom in/out, 100% and fit, and per-project/root-or-composite viewport restore are implemented; pointer-anchored/pinch zoom remains | Add pointer-anchored and pinch zoom | P1 | M | 60 fps target; focus and hit testing remain aligned at all zooms |
| Graph layout persistence | Per-project canvas pan/zoom and node layout | Client-local `CanvasLayoutStore` | **Partial**: a validated version-1 Tauri app-data store atomically persists root/composite viewports by project and reports corrupt, oversized, or unsupported data; manual node positioning is not implemented | Extend the same schema only if user-positioned nodes are added | P1 | M | Restart restores viewport; invalid entries fail visibly without overwriting them |
